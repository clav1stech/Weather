...
- **Ne persister un run frais que s'il est complet** (`filter_fresh_rows`/`_meets_persist_horizon` dans Forecast.py) : portée réelle ≥ `horizon_h − PERSIST_HORIZON_TOLERANCE_H` (ou `MIN_PERSIST_HORIZON_H` si `horizon_h` inconnu, ex. GEM), sinon laissé de côté (ancien run complet conservé, comme un cycle inchangé). Deux causes distinctes, même traitement : un modèle à horizon plein 4×/j (AIFS, GEFS) juste **en cours de calcul** (se résout au poll suivant) vs un cycle **nativement plus court** par construction (ex. ECMWF ENS à 6Z/18Z ≈ 144 h, jamais 360 h — ce cycle ne sera donc simplement **jamais persisté**, volontairement). On continue de checker le modèle à chaque poll (Metadata API + heuristique) ; seule la persistance est conditionnée.
- **Ne jamais régresser un (run_date, modèle) déjà en base** : `persist()` appelle `_drop_regressions` en dernier rempart, indépendant de `filter_fresh_rows` — si `fresh` contient un couple (run_date, modèle) déjà stocké mais avec une portée réelle (dernière échéance valide − run_date) INFÉRIEURE à celle déjà en base (glitch API, réponse tronquée à un poll), ce couple est écarté de `fresh` avant fusion : le run existant, plus complet, n'est jamais remplacé par une version régressive. Un `run_date` nouveau pour ce modèle n'est pas concerné (rien à régresser, c'est `filter_fresh_rows` qui juge sa complétude).
