...
def load_existing():
    ...
    if _store_source_active():
        return load_existing_from_store(C.DB_PATH, C.SCHEMA)
...
# apps/snow/pipeline/store_mirror.load_existing, même règle).
...
def load_existing_from_store(db_path, schema):
    ...
    base nulle, puis réécrirait la partition du mois courant avec le seul run
