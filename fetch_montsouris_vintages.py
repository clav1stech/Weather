...
def load_existing():
    ...
    if _store_source_active():
        return load_existing_from_store(C.DB_VINTAGE_PATH, C.VINTAGE_SCHEMA)
...
def main():
    ...
    existing = load_existing()
    combined, n_new = persist(fresh, existing, now=fetched_at)
    ...
    if os.environ.get("WEATHER_STORE_WRITE", "").strip().lower() not in ("", "0", "false"):
        mirror_to_store(existing, combined, C.DB_VINTAGE_PATH, "valid_time")
